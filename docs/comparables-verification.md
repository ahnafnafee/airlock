...
Airlock itself also has an important present-tense limitation: its README says server-held transfers work end to end, but directly delivered chunks cannot yet be assembled and saved on the receiver; reliable receipt currently requires **Hold on the server if I go offline** ([Airlock README](../README.md#status)).
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Airlock** | **Yes** — `WhoIs()` plus device registry | **PWA** | **Yes** — browser-sealed AES-256-GCM | Designed yes; receive assembly unfinished | **Yes** when server-hold is selected; otherwise locally staged until overlap | **All three**, content-defined | **Yes**, one Go binary |
