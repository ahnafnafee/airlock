# Deploy Airlock on a VPS and add an Android device (detailed)

> **Most people do not need this page.** If you just want Airlock running on a
> machine you already own, follow [Set up a server](./deployment.md), which is
> three commands. This page is the long version: a rented VPS, Docker, the
> embedded tailnet node, and the operational detail that goes with running it
> somewhere you cannot walk over to.
...
<p align="center">
  <img src="./assets/deployment-shape.svg" alt="An Android phone and a trusted desktop both join your Tailscale network. Airlock runs in Docker on the VPS as its own node on that network, with a single Docker volume holding its state. Nothing is exposed to the public internet." width="760">
</p>
